...
test('navigates core mobile flows', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Conversor técnico' })).toBeVisible()
  await expect(page.getByText('3,49')).toBeVisible()
  await page.goto('/vacuum')
  await expect(page.getByRole('heading', { name: 'Vacío y estabilidad' })).toBeVisible()
  await expect(page.getByText('500 μm')).toBeVisible()
