...
export function PsychrometricsPage() {
  ...
  const canSave = Boolean(calculation)

  const loadHistory = () => {
    void listRecentCalculationHistory(8, 'dry-bulb-relative-humidity').then(setHistory)
  }
  ...
  const calculate = () => {
    try {
      ...
      setCalculation(null)
      setMessage(error instanceof Error ? error.message : 'No se pudo calcular la psicrometría.')
    }
  }
  ...
  const reset = () => {
    ...
    setPressurePa(String(Math.round(atmospherePa)))
    setCalculation(null)
    setMessage('')
  }
  ...
  return <main className="sz-screen sz-pt-screen">
    <PageTitle eyebrow="Climatización" title="Psicrometría" description="Propiedades de aire húmedo desde temperatura seca, humedad relativa y presión atmosférica." />

    <div className="sz-mode-switch" aria-label="Pestañas de psicrometría">
    ...
    {tab === 'calcular' && <>
      <section className="sz-panel sz-form">
        <div className="sz-two-columns"><label>Temperatura seca °C<input inputMode="decimal" value={dryBulb} onChange={(event) => setDryBulb(event.target.value)} /></label><label>Humedad relativa %<input inputMode="decimal" value={relativeHumidity} onChange={(event) => setRelativeHumidity(event.target.value)} /></label></div>
        <label>Presión atmosférica Pa<input inputMode="decimal" value={pressurePa} onChange={(event) => setPressurePa(event.target.value)} /></label>
      ...
      <section className="sz-result"><small>Resultado principal</small><strong>{calculation ? `${formatNumber(calculation.result.dewPointC, 1)} °C` : '—'}</strong><span>Punto de rocío</span>{calculation && <div className="sz-data-list">{resultRows}</div>}<div className="sz-button-row"><button className="sz-button secondary" type="button" disabled={!canSave} onClick={save}><Save />Guardar historial</button><button className="sz-button ghost" type="button" disabled={!canSave} onClick={() => setTab('interpretar')}>Interpretar</button></div></section>
    </>}
    ...
    {tab === 'aprender' && <section className="sz-panel"><h2>Cómo se calcula</h2><p>PsychroLib calcula propiedades de aire húmedo en SI a partir de temperatura seca, humedad relativa y presión atmosférica.</p><div className="sz-data-list"><p><span>Fuente</span><strong>PsychroLib MIT / ASHRAE Fundamentals</strong></p><p><span>Entrada mínima</span><strong>T seca + HR + presión</strong></p><p><span>Uso</span><strong>Orientativo; validar con proyecto y medición calibrada</strong></p></div></section>}
    ...
    <Notice tone="warning"><p>Resultado orientativo. En diseño o puesta en marcha prevalecen proyecto, normativa, fabricante e instrumentos calibrados.</p></Notice>
  </main>
}
